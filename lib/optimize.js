...
const IMAGE_LINK_RE = /https?:\/\/[^\s"'<>]+\.(?:png|jpe?g|gif|webp)(?:\?[^\s"'<>]*)?/gi;
function extractImageLinks(text) {
	return text.match(IMAGE_LINK_RE) ?? [];
}
function buildOptimizePrompt(text, images = [], systemPrompt = SYSTEM_PROMPT) {
	const content = [
		{
			type: "text",
			text
...
async function callDeepSeekOptimize(text, deps = {}) {
	...
	if (typeof content !== "string" || content.trim() === "") throw new OptimizeError("empty-response", `DeepSeek API returned no content${typeof choice?.finish_reason === "string" && choice.finish_reason !== "" ? ` (finish_reason: ${choice.finish_reason})` : ""}`);
	return content;
}
//#endregion
export { BASE_URL, IMAGE_LINK_RE, MAX_OUTPUT_TOKENS, MODEL, OptimizeError, SYSTEM_PROMPT, buildOptimizePrompt, callDeepSeekOptimize, extractImageLinks };
